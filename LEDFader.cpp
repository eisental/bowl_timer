...
void LEDFader::set_pin(uint8_t pwm_pin) { pin = pwm_pin; }
uint8_t LEDFader::get_pin() { return pin; }
...
uint8_t LEDFader::get_value() { return color; }

uint8_t LEDFader::get_target_value() { return to_color; }

// Set curve to transform output
void LEDFader::set_curve(curve_function c) { curve = c; }

// Get the current curve function pointer
LEDFader::curve_function LEDFader::get_curve() { return curve; }
...
void LEDFader::fade(uint8_t value, unsigned int time) {
  ...
  // Figure out what the interval should be so that we're chaning the color by
  // at least 1 each cycle (minimum interval is MIN_INTERVAL)
...
uint8_t LEDFader::get_progress() { return round(percent_done); }
